<script lang="ts">
	...
	let startHour = $derived.by(() => {
		...
		// Opt: Using native Date methods inside mapping loop is faster than instantiating dayjs objects
	...
	let endHour = $derived.by(() => {
		...
		return Math.max(
			...weekEvents.map((e) => {
				// Opt: Using native Date methods to determine hours is faster than dayjs
		...
		<div class="relative" style="height:{hours.length * 48}px;">
			...
			{#each weekEvents as event (event.start.valueOf() + event.title)}
				{@const startDayIdx = weekDaysArr.findIndex(
					(day) =>
						day.date() === event.start.getDate() &&
						day.month() === event.start.getMonth() &&
