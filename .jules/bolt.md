...
## 2026-04-22 - Pre-compute event formats and sorts in Timeline

**Learning:** Running `O(n log n)` sorts or creating `dayjs` instances in Svelte 5 templates within reactivity blocks that are frequently triggered by `setInterval` causes significant CPU overhead.
**Action:** Pre-compute formats and pre-sort lists once inside `$derived.by` blocks to avoid doing these operations repeatedly on every interval tick.
