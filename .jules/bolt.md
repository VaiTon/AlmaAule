...
## 2025-04-18 - [Optimize Event Processing in Timeline.svelte]

**Learning:** Inline `.sort()` calls and expensive `dayjs` formatting inside Svelte templates (especially inside `{#each}` loops dependent on fast-changing input like timers) trigger frequent recalculations, causing measurable CPU overhead and layout thrashing.
**Action:** Move sorting and formatting operations into `$derived` blocks to pre-compute them once when the data changes, keeping template rendering lean.
